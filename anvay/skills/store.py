...
from anvay.skills.models import AppliesTo, Provenance, Skill, SkillCoverage
...
def _build_skill(meta: dict, body: str) -> Skill:
    ...
    if isinstance(metadata, dict) and metadata.get("anvay_product"):
...
def _dump_frontmatter(skill: Skill) -> dict:
    return {
        ...
        "metadata": {
            "anvay_product": skill.product,
            "anvay_tier": skill.tier,
            "anvay_parent": skill.parent,
            "anvay_related": skill.related,
            "anvay_coverage": skill.coverage.model_dump(),
            "anvay_version": skill.version,
            "anvay_confidence": skill.confidence,
            "anvay_eval_status": skill.eval_status,
            "anvay_eval_summary": skill.eval_summary,
            "anvay_eval_failures": skill.eval_failures,
            "anvay_quality_score": skill.quality_score,
            "anvay_signals_used": skill.signals_used,
            "anvay_applies_to": skill.applies_to.model_dump(),
            "anvay_provenance": skill.provenance.model_dump(),
        },
    }
...
def _build_agent_skill(meta: dict, metadata: dict, body: str) -> Skill:
    return Skill(
        name=meta["name"],
        description=str(meta.get("description", "")),
        product=str(metadata["anvay_product"]),
        tier=str(metadata.get("anvay_tier") or "domain"),
        parent=metadata.get("anvay_parent"),
        related=list(metadata.get("anvay_related") or []),
        coverage=SkillCoverage(**(metadata.get("anvay_coverage") or {})),
        version=int(metadata.get("anvay_version", 1)),
        confidence=float(metadata.get("anvay_confidence", 0.0)),
        eval_status=str(metadata.get("anvay_eval_status", "not_run")),
        eval_summary=str(metadata.get("anvay_eval_summary", "")),
        eval_failures=list(metadata.get("anvay_eval_failures") or []),
        quality_score=float(metadata.get("anvay_quality_score", 0.0)),
        signals_used=list(metadata.get("anvay_signals_used") or []),
        applies_to=AppliesTo(**(metadata.get("anvay_applies_to") or {})),
        provenance=Provenance(**metadata["anvay_provenance"]),
        body=body,
    )

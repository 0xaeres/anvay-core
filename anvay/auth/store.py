"""SQLite auth store for deployed Anvay instances."""
...
class AuthStore:
    def __init__(self, db_path: Path, *, secret_key: str | None = None):
        ...
        self.secret_key = secret_key or os.getenv("ANVAY_SECRET_KEY") or ""
    ...
    def bootstrap_admin_from_env(self) -> None:
        email = _normalize_email(os.getenv("ANVAY_BOOTSTRAP_ADMIN_EMAIL") or "")
        password = os.getenv("ANVAY_BOOTSTRAP_ADMIN_PASSWORD") or ""
    ...
    def hash_session_token(self, token: str) -> str:
        if not self.secret_key:
            raise AuthError("ANVAY_SECRET_KEY is required for auth sessions")
        digest = hmac.new(

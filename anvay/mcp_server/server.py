"""Anvay MCP server (stdio transport).
...
is served against a single product configured at launch:
  ...
  uv run python -m anvay.mcp_server.server --product <your-product-id>
...
from anvay.mcp_server import tools as nx_tools
...
def _build_server(*, product: str, config: AnvayConfig) -> tuple[Server, nx_tools.ToolState]:
    ...
    async def list_tools() -> list[Tool]:
        return [
            ...
            Tool(
                name="report_outcome",
                description="Tell Anvay whether a skill helped. Feeds staleness tracking.",
                inputSchema={
                    "type": "object",
                    "properties": {
    ...
    async def list_resources() -> list[Resource]:
        return [
            Resource(
                uri="anvay://meta-skill",
                name="How to Use Anvay",
    ...
    async def read_resource(uri: str) -> str:
        if uri == "anvay://meta-skill":
            return await _render_meta_skill(state)
        if uri == "anvay://hierarchy":
            tree = await nx_tools.skill_hierarchy(state)
            return json.dumps(tree, indent=2)
        if uri.startswith("anvay://skills/"):
            name = uri.removeprefix("anvay://skills/")
            return await nx_tools.skill_markdown(state, name=name)
        if uri.startswith("anvay://corpus/"):
            requested = uri.removeprefix("anvay://corpus/")
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Anvay MCP server (stdio).")
    p.add_argument(
        "--product",
        default=os.environ.get("ANVAY_PRODUCT"),
        required=not os.environ.get("ANVAY_PRODUCT"),
        help="Product ID to serve. Set via --product or ANVAY_PRODUCT env var.",
    )
    p.add_argument(
        "--config",
        default=os.environ.get("ANVAY_CONFIG", "anvay.yaml"),
        help="Path to anvay.yaml.",
    )
    return p.parse_args()

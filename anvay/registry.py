...
def _get_cipher() -> TokenCipher | None:
    key = os.environ.get("ANVAY_TOKEN_KEY")
    if not key:
        return None
    try:
        return TokenCipher(key)
    except TokenCipherError:
        log.exception("ANVAY_TOKEN_KEY is set but invalid")
        return None
...
def _encrypt_config(config: dict, cipher: TokenCipher | None) -> dict:
    if not cipher and any(_is_secret_key(k) and isinstance(v, str) and v for k, v in config.items()):
        raise TokenCipherError(
            "ANVAY_TOKEN_KEY is required to store connector secrets; generate one "
    ...
    for k, v in config.items():
        if isinstance(v, str) and v.startswith("enc:"):
            if cipher:
                ...
                # Key was encrypted but ANVAY_TOKEN_KEY is now missing.
                log.warning(
                    "Encrypted value for %r but ANVAY_TOKEN_KEY not set; returning empty", k
                )
                out[k] = ""
        else:
            out[k] = v
...
-- Runtime-added sources. Merged with anvay.yaml connectors at read time.

...
from anvay.api.authz import auth_enabled, prod_enabled
from anvay.api.deps import get_auth_store, get_config_dep, get_registry
from anvay.api.routes import (
    agent,
    auth,
    council,
    dashboard,
...
app = FastAPI(
    title="Anvay API",
    description="Backend for the Anvay context engine. See ENGINEERING.md §8.",
    version="0.0.1",
    lifespan=lifespan,
)
...
async def _authenticate_request(request: Request) -> JSONResponse | None:
    ...
    if request.method in {"POST", "PUT", "PATCH", "DELETE"} and not _is_csrf_exempt(
        ...
        header_token = request.headers.get("x-anvay-csrf", "")
...
def _validate_production_config() -> None:
    ...
    if not cfg.skills_repo:
        missing.append("skills_repo/ANVAY_SKILLS_REPO")
    if missing:
        raise RuntimeError(

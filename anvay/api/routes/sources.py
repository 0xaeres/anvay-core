...
1. `anvay.yaml` `connectors:` block (declarative, baked in at deploy time).
...
from anvay.api.authz import assert_product_access, local_fs_enabled, rate_limit
...
from anvay.connectors.confluence import ConfluenceSource, confluence_config_from_source
from anvay.connectors.jira import JiraSource, jira_config_from_source
...
from anvay.setup.bootstrap import _authenticated_clone_url
...
def _config_sources(config: AnvayConfig, product_id: str) -> list[dict]:
    out: list[dict] = []
...
class _CanonicalProgressSource:
    ...
    Implements the `_Source` protocol from anvay.ingest.pipeline.
    """
...
async def _sync_source_contents(
    *,
    ...
    try:
        ...
        if repo_map_symbols:
            try:
                from anvay.retrieval.repomap import RepoMap, repomap_path_for, save_repo_map
...
async def _ingest_root(
    *,
    ...
    rm = None
    try:
        from anvay.retrieval.repomap import extract_repo_map
...
async def _clone_github_repo(
    ...
    tmp = Path(tempfile.mkdtemp(prefix="anvay-ingest-"))
    clone_path = tmp / "repo"

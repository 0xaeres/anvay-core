...
See `anvay/setup/` for the actual orchestration. The UI calls `GET /setup/status`
...
from anvay.api.authz import require_admin
from anvay.api.deps import get_config_dep, get_setup_kv, resolve_skills_repo_url
from anvay.config import AnvayConfig
from anvay.setup import BootstrapError, SetupKV, bootstrap_skills_repo
from anvay.skills.git import ensure_checkout
...
async def setup_skills_repo(
    ...
    repo_name: str = Body("anvay-skills", embed=True),
    ...
    create_token = os.environ.get("ANVAY_SKILLS_REPO_TOKEN") or ""
    checkout_token = _skills_repo_token()
    if mode == "create" and not create_token:
        raise HTTPException(
            status_code=400,
            detail=(
                "ANVAY_SKILLS_REPO_TOKEN env var is required to create a skills repo; "
...
def _skills_repo_token() -> str:
    return os.environ.get("ANVAY_SKILLS_REPO_TOKEN") or os.environ.get("GITHUB_TOKEN") or ""

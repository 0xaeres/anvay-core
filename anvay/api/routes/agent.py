...
from anvay.graph.models import GraphRAGAnswer, GraphRAGMessage
from anvay.mcp_server.tools import ToolState, ask_product_graph
from anvay.registry import Registry
from anvay.retrieval.evidence import EvidenceMode
...
async def ask_product_agent(
    ...
    """Ask Anvay an arbitrary product question through graph-filtered RAG."""
    ...
    if _is_greeting(body.message):
        answer = GraphRAGAnswer(
            ...
            query=body.message,
            answer="Hey. Ask me about Anvay code, retrieval, ingestion, council flow, or UI behavior.",
            confidence=1.0,
...
def _config_for_chat_model(config: AnvayConfig, model_name: str | None) -> AnvayConfig:
    if not model_name:
        return config
...
def _agent_sessions(config: AnvayConfig) -> AgentSessionStore:
